...
#update- exclude nz and au passport from analysis
ts_df_total$Residence <- as.numeric(ts_df_total$Residence)
ts_df_total$Student <- as.numeric(ts_df_total$Student)
ts_df_total$Work <- as.numeric(ts_df_total$Work)
ts_df_total$Other <- as.numeric(ts_df_total$Other)
ts_df_total$TOTALnoNZAU <- ts_df_total$Residence + ts_df_total$Student + ts_df_total$Visitor + ts_df_total$Work + ts_df_total$Other


start_year <- 2003
start_month <- 7
ts_visitor <- ts(ts_df_total$Visitor, start = c(start_year, start_month), frequency = 12)
ts_total <- ts(ts_df_total$TOTALnoNZAU, start = c(start_year, start_month), frequency = 12)

# ts plot
plot(ts_visitor, col = "blue", ylim = range(c(ts_visitor, ts_total)),
     ylab = "Number of Visitors", xlab = "Year", main = "Time Series of Foreign Visitors to New Zealand",
     xaxt = "n", yaxt = "n")
...
#recovery percentage of visitor visa
